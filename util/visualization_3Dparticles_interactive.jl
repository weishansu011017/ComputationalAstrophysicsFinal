...
function main()
    ...
    xyztitle = Observable((Float32[], Float32[], Float32[], @sprintf("t = %.3f [Myr], Error = %.1f %% (%s)", 0.0, 0.0, SimulationTag[])))
    ## Split trigger
    points = map(xyztitle) do (x, y, z,  _)
        Point{3, Float32}.(x, y, z)
    end
    title_obs = @lift($xyztitle[3])
    ...
    z_array = zeros(Float32, testdata.params["N"], length(files))
    ## Lower
    ...
    for (i,file) in enumerate(files)
        ...
        vz = data.Table[!,"vz"]
        m = data.Table[!,"m"]
        KE_array[i] = sum(0.5 .* m .* (vx.^2 .+ vy.^2 + vz.^2))
    ...
    # Upper panel
    axu = Axis3(fig[1:2,1], xlabel = xulabel, ylabel = yulabel, zlabel = zulabel, limits = ((-0.25, 0.25), (-0.25, 0.25), (-0.25, 0.25)),  title = title_obs, xlabelsize = 20, ylabelsize = 20, zlabelsize = 20, titlesize = 22, xticklabelsize = 14, yticklabelsize = 14, zticklabelsize = 14)
    # Lower panel
    ...
    # First upper panel
    xyztitle[] = (x_array[:,1], y_array[:,1], z_array[:,1], @sprintf("t = %.3f [Myr], Error = %.1f %% (%s)", 0.0, 0.0, SimulationTag[]))
